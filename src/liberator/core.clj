...
(defn gen-cache-control [context]
  (if-let [f (get-in context [:resource :cache-control])]
    (f context)))
...
(defn decide
  "Run a test and call either 'then' or 'else' depending on the result of the test"
  [name test then else {:keys [resource request] :as context}]
  ;If we have been passed a test function, or we have a handler in the resource for a given test function
  (if (or (fn? test) (contains? resource name))
    (try
      (let [ftest (or (resource name) test)
            ...
            decision (ftest context)                        ;run the test on the context
        ...
        ;run either the "then" or "else" function depending on the result with the updated context
...
(defmacro defdecision
  "Create a function that will execute a test, then call another function based on the result.
  Can be called with the name of the keyword in the resource that will yield the test function or with
  a test function"
  ([name then else]
...
(defn build-vary-header [{{:keys [media-type charset language encoding]} :representation :as context}]
  (let [vary-fn (get-in context [:resource :vary])]
...
(defn run-handler [name status message
  ...
  (let [context (merge {:status status :message message} context)
        response (merge-with
                   ...
                   ;cache-control
                   (when-let [cache-control (gen-cache-control context)]
                        ...
                        (-> {}
                            ...
                            (set-header-maybe "Content-Encoding"
                                              (let [e (:encoding representation)]
                                                (if-not (= "identity" e) e)))
                            (set-header-maybe "Vary" (build-vary-header context)))}
...
(defmacro ^:private defhandler
  "Create a function that will process the context to produce a Ring response map"
  [name status message]
  `(defn ~name [context#]
...
(defn run-resource
  "Run a Ring request through the functions in the resource to produce a Ring response map"
  [request kvs]
  (try
    ;To begin the decision-making process, send the request to the first fn in the chain.
    ;Make sure default functions are set for any that the user didn't specify
...
(defmacro defresource
  "Create a Ring handler function that will use the key -> fn map to return a response map"
  [name & kvs]
  (if (vector? (first kvs))
    (let [args (first kvs)
          kvs (rest kvs)]
